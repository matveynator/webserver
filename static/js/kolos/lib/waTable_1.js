var waTable = function(tagId,varName){
    ...
    /*
    this.readPkg = function(pkg_){
        this.fields =           pkg.package['fields'];
        if(pkg.package['params']['fields_hidden'] == null)
            this.fields_hidden =    {}
        else
            this.fields_hidden =    pkg.package['params']['fields_hidden'];
        this.data =             pkg.package['data'];
        this.count =            pkg.package['count'];
        this.offset =           pkg.package['offset'];
        this.rows =             pkg.package['rows'];
        if(pkg_.package['params']!=null)
            this.params = pkg_.package['params'];
    ...
    */
    this.getPkg = function(){
        ...
        var pkg = pkg_table().instance()
                             ...
                             .offset(this.offset)
                             .rows(this.rows);
        pkg.package['params'] = this.params;
        */
    ...
    this.loadData = function(){
        ...
        tc.ajax(Self.address,{async:true,data:'pkg='+Self.getPkg().data({}).fields({}).asJSON()}).success(function(data){
            //Self.readPkg(pkg_table().instance().parse(data));
            Self.readPkg(pkg.instance(data));
        });
        */
    }
    ...
    this.dataDraw = function(){        
        ...
        for(key in this.fields){ // заголовок
            if(this.fields_hidden[key]==null)
        ...
        for(var index in this.data){
            ...
            // выводим ячейки
            for(var cell in this.data[index]){
                if(this.fields_hidden[cell]==null)
                    line += "<td style=\"vertical-align: middle\">" + this.cellDraw(this.data[index]["id"],index,cell) + "</td>";
            }
            // выводим опции
